...
function Technologies() {
  ...
  return (
    <div className="mx-auto py-32 px-4 md:px-16 lg:px-60" id="TechnologiesUsed">
      <motion.p
        className="text-2xl md:text-4xl my-2 font-bold text-start text-white"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        transition={{ duration: 1.0 }}
      >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-6 justify-items-center">
        {techStack.map((tech, index) => (
          <motion.div
            key={index}
            className="w-24 h-24 sm:w-32 sm:h-32 p-4 bg-black rounded-lg shadow-lg flex items-center justify-center"
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
